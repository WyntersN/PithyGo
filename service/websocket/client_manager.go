...
func (manager *ClientManager) GetUserList(appId uint32) (userList []string) {
	...
	service.LOG.Sugar().Info("GetUserList len:", len(manager.Users))
...
func (manager *ClientManager) EventRegister(client *Client) {
	...
	service.LOG.Sugar().Info("EventRegister 用户建立连接", client.Addr)
...
func (manager *ClientManager) EventLogin(login *login) {
	...
	service.LOG.Sugar().Info("EventLogin 用户登录", client.Addr, login.AppId, login.UserId)

	orderId := helper.GetOrderIdTime()
	SendUserMessageAll(login.AppId, login.UserId, orderId, models.MessageCmdEnter, "哈喽~"+client.RealName)
}
...
func (manager *ClientManager) EventUnregister(client *Client) {
	...
	service.LOG.Sugar().Info("EventUnregister 用户断开连接", client.Addr, client.AppId, client.UserId)
...
func ClearTimeoutConnections() {
	...
	for client := range clients {
		if client.IsHeartbeatTimeout(currentTime) {
			service.LOG.Sugar().Info("心跳时间超时 关闭连接", client.Addr, client.UserId, client.LoginTime, client.HeartbeatTime)
...
func GetUserList(appId uint32) (userList []string) {
	service.LOG.Sugar().Info("获取全部用户", appId)
...
func AllSendMessages(appId uint32, userId string, data string) {
	service.LOG.Sugar().Info("全员广播", appId, userId, data)
