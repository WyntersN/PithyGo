...
type UserOnline struct {
	...
	RealName      string `json:"realname"`      // 用户Id
...
// 用户登录
func UserLogin(accIp, accPort string, appId uint32, userId, realname, addr string, loginTime uint64) (userOnline *UserOnline) {
...
func (u *UserOnline) IsOnline() (online bool) {
	...
	if u.HeartbeatTime < (currentTime - heartbeatTimeout) {
		service.LOG.Sugar().Info("用户是否在线 心跳超时", u.AppId, u.UserId, u.HeartbeatTime)

		return
	}

	if u.IsLogoff {
		service.LOG.Sugar().Info("用户是否在线 用户已经下线", u.AppId, u.UserId)
