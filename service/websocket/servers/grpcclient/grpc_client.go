...
func SendMsgAll(server *models.Server, seq string, appId uint32, userId string, cmd string, message string) (sendMsgId string, err error) {
	...
	if err != nil {
		service.LOG.Sugar().Info("连接失败", server.String())
	...
	if err != nil {
		service.LOG.Sugar().Info("给全体用户发送消息", err)

		return
	}

	if rsp.GetRetCode() != common.OK {
		service.LOG.Sugar().Info("给全体用户发送消息", rsp.String())
	...
	service.LOG.Sugar().Info("给全体用户发送消息 成功:", sendMsgId)
...
func GetUserList(server *models.Server, appId uint32) (userIds []string, err error) {
	...
	if err != nil {
		service.LOG.Sugar().Info("获取用户列表 发送请求错误:", err)

		return
	}

	if rsp.GetRetCode() != common.OK {
		service.LOG.Sugar().Info("获取用户列表 返回码错误:", rsp.String())
		err = errors.New(fmt.Sprintf("发送消息失败 code:%d", rsp.GetRetCode()))
	...
	service.LOG.Sugar().Info("获取用户列表 成功:", userIds)
...
func SendMsg(server *models.Server, seq string, appId uint32, userId string, cmd string, msgType string, message string) (sendMsgId string, err error) {
	...
	if err != nil {
		service.LOG.Sugar().Info("发送消息", err)

		return
	}

	if rsp.GetRetCode() != common.OK {
		service.LOG.Sugar().Info("发送消息", rsp.String())
		err = errors.New(fmt.Sprintf("发送消息失败 code:%d", rsp.GetRetCode()))
	...
	sendMsgId = rsp.GetSendMsgId()
	service.LOG.Sugar().Info("发送消息 成功:", sendMsgId)
